...
class LichessClient implements Client {
  ...
  Future<StreamedResponse> send(BaseRequest request) async {
    ...
    // Add cookies to request if user is logged in
    if (session != null) {
      try {
        final cookieJar = await _ref.read(cookieJarProvider.future);
        final cookieManager = CookieManager(cookieJar);
        final cookieHeader = await cookieManager.getCookieHeader(request.url);
        if (cookieHeader != null) {
          request.headers['Cookie'] = cookieHeader;
        }
      } catch (e) {
        _logger.warning('Failed to load cookies: $e');
      }
    }
      ...
      if (session != null) {
        try {
          ...
          if (setCookieHeader != null) {
            final cookieJar = await _ref.read(cookieJarProvider.future);
            final cookieManager = CookieManager(cookieJar);
            await cookieManager.saveFromSetCookieHeader(request.url, setCookieHeader);
          }
        } catch (e) {
          _logger.warning('Failed to save cookies: $e');
        }
      }
  ...
  /// Checks if the session is still valid by verifying the cookie.
  /// If session is invalid (401), delete the session.
  Future<void> _checkSessionToken(AuthSessionState session) async {
    // With cookie-based auth, if we get a 401, the session cookie is invalid
    // Simply delete the session and let the user re-authenticate
    _logger.fine('Session is not active (401 received). Deleting it.');
